...
SELECT D.name [PAÍS], E.dateUpdate [ATUALIZAÇÃO], H.amount [TOTAL], I.amount [MORTES], J.amount [RECUPERADOS], K.amount [ATIVOS], L.name [ARQUIVO], M.recordedDt [PROCESSAMENTO] FROM Daily AS A
...
SELECT D.name [PAÍS], /*E.dateUpdate [ATUALIZAÇÃO], H.amount [TOTAL],*/ (MAX(I.amount) - MIN(I.amount)) [MORTES]/*, J.amount [RECUPERADOS], K.amount [ATIVOS], L.name [ARQUIVO], M.recordedDt [PROCESSAMENTO]*/ FROM Daily AS A
	INNER JOIN CountryRegion AS D
		ON D.id = A.CountryRegionId
	INNER JOIN LastUpdate AS E
		ON E.id = A.LastUpdateId
